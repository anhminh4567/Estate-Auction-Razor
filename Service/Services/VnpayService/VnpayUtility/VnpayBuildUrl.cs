...
{
	public class VnpayBuildUrl
	{
		private int _amount { get; set; } = 0;
		private string _bankCode;
		private int _transactionDuration = 5;// minute
		private readonly IUnitOfWork _unitOfWork;
		public VnpayBuildUrl(IUnitOfWork unitOfWork)
		{
			_unitOfWork = unitOfWork;
		}

		//protected void btnPay_Click(object sender, EventArgs e)
		public async Task<string> btnPay_Click(HttpContext httpContext, Account acc, int amount, string bankcode = null)
		{
			_amount = amount;
			_bankCode = bankcode;
			...
			//	return null;
			//}
			var currentDateTime1 = order.CreatedDate.Year.ToString() +
					order.CreatedDate.Month.ToString() +
					order.CreatedDate.Date.ToString() +
					order.CreatedDate.Hour.ToString() +
					order.CreatedDate.Minute.ToString() +
					order.CreatedDate.Second.ToString();
